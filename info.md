...
## Key Achievements (from current todo)
- API endpoints: `/publish/:topic` and `/notify`
- Kafka producer keyed by user; workers consuming `notifications.email` and `notifications.sms`
- Email worker with retries and DLQ; metrics and JSON logs
- SMS worker with retries, DLQ, per-tenant rate limit; metrics
- Idempotency via DB unique index + cache
- API key auth; short and long-term rate limiting
- PostgreSQL schema for notifications, delivery attempts, API keys, templates; search and redrive endpoints
- Templates with Go templates and localization
- Policy engine mapping topic → channels honoring preferences
